...
> Notion 워크스페이스 운영 규칙 → `context/notion-rules.md`
...
| **세무/회계 질의** | `/tax` skill | Claude(해석) | vault 페르소나+지식 기반, SSH→M1 |
| **전문직 Q&A** | `/expert` skill | — | 자동 라우팅 (accounting/legal/deal/audit 등) |
| **콘텐츠 관리** | `/content` skill | — | content-automation 파이프라인 조회/관리 |
| **이미지 직접 생성** (빠른 썸네일·간단 에셋) | Gemini 웹 (수동) | — | gemini.google.com — 플랜 포함, CLI/MCP 미지원 |
| **영상 편집** | video_edit.sh (FFmpeg) | Gemini(ai 명령) | `brew install ffmpeg` 필요 |
