...
| Condition | Action |
|---|---|
| 50+ lines of code to write | `Bash("codex exec --skip-git-repo-check \"task\"")` |
| 4+ files to create/modify | `Bash("codex exec --skip-git-repo-check \"task\"")` |
...
Examples:
- "지뢰찾기 게임 만들어줘" → Python ~100줄 → `codex exec --skip-git-repo-check "task"`로 위임
