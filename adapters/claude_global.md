...
- **호출 방식: 반드시 Bash로 Gemini CLI 직접 호출** — `gemini-researcher` Agent 서브에이전트 사용 금지
  ```bash
  gemini --yolo -m gemini-2.5-flash -p "리서치 내용"
  ```
  - Agent 서브에이전트는 실제 리서치 대신 내부 태스크 위임만 반복하는 버그 있음 (확인됨 2026-03-21)
