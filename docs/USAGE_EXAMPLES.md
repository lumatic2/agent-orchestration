...
```bash
# 세무 질의 (planby 컨텍스트 포함)
bash scripts/tax_agent.sh "TIPS 정부지원금 회계처리 방법. 국고보조금으로 영업외수익 처리하는 경우 분개와 공시 요건" --planby

# CPA 법인세 계산
bash scripts/tax_agent.sh "법인세 이월결손금 활용 방법. 결손금 발생 연도, 공제 한도(80%), 15년 이월 계산 예시" --pro

# 법률 질의
bash scripts/expert_agent.sh lawyer "스타트업 투자계약서의 주요 독소조항. 우선청산권, 반희석화, 드래그얼롱 조건별 실무 리스크"
```
...
```bash
# 멀티챕터 콘텐츠 생성
bash scripts/content_pipeline.sh init "커피의 역사와 문화" --chapters 18
bash scripts/content_pipeline.sh write 1  # 챕터별 순차 실행
bash scripts/content_pipeline.sh compile  # 전체 병합

# 식품 관련 페이지 시리즈
bash orchestrate.sh gemini-pro "한식 김치 스토리텔링 콘텐츠. 역사, 재료, 제조 과정, 문화적 의미, 세계화 스토리. 블로그 3000자" food-pages-completion
```
...
# 세무 질의 (RAG 컨텍스트 포함)
bash scripts/tax_agent.sh "TIPS 기술기여도 계산 방법" --planby --pro
```
...
```bash
# 이미지 프롬프트 생성 (DALL-E / MJ / SD 3종 동시)
bash scripts/image_agent.sh "건축 AI 스타트업 로고" --type 로고 --ratio 1:1

# 영상 편집 (FFmpeg 없어도 명령어 생성)
