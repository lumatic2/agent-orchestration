...
### 12. `gemini_run` content-less confabulation — capacity 와 독립된 fabrication 모드 (2026-04-09, Step 4a Session 3 실증)

**증상**: Gemini pro 가 **capacity 정상** 상태에서도 grounding 이 "찾지 못함" 을 인정하지 않고 training data 에서 가장 그럴듯한 citation 패턴을 조합해 fabricated URL 을 emit. #10 (capacity exhaustion → fabrication) 과 별개의 failure mode — 서버 부하와 무관하게 발생한다.

**Root cause** (추정): Gemini CLI 의 grounding fallback behavior 가 "결과 없음" 을 null 로 반환하지 않고 training-data-조합 응답을 생성. 특히 존재하지 않는 reference (예: "NIAH 를 옹호하는 counter-paper") 를 요구받을 때 발동한다.

**실증 데이터** (Session 3, arxiv-heavy 주제, capacity 여유 확인 후 실행):

| claim | Gemini 가 emit 한 URL | 실제 상태 | 탐지 heuristic |
|---|---|---|---|
| A2 | `arxiv.org/abs/2604.03000` | URL 실존, 그러나 수학 논문 — claim 과 무관 | arxiv ID/title mismatch |
| B1 | `aclanthology.org/2025.tacl-1.9/` | URL 실존, "Transformers as Transducers" — claim 과 무관 | URL exists but unrelated content |
| B3 | `arxiv.org/abs/2507.30` | invalid arxiv id (digit count 부족) | YYMM.NNNNN format violation |
| B4 | `arxiv.org/abs/2604.03` | invalid/truncated arxiv id | format violation |

4건 모두 같은 라운드, 같은 branch 구성 (3 sequential, 25초 간격), 모든 branch 본문 정상 반환, 0 timeout, 0 429. **즉 capacity 문제 없이 fabrication 발생.**

**#10 과의 차이**:
- #10: capacity exhaustion → CLI 가 retry loop 후 training data 로 fallback. 가짜 slug 패턴 (`0f1b1b1b1b1b`) + 날짜 필터 위반 빈발.
- #12: capacity 정상 → CLI 가 "못 찾음" 을 null 로 내지 않고 형식 맞춰 가짜 생성. arxiv id format violation + 실존 URL 의 content mismatch 빈발.

**특히 위험한 패턴**: Branch B 처럼 **negative finding (존재하지 않는 literature)** 을 요구받을 때 confabulation 이 집중된다. "NIAH 를 옹호하는 counter-paper 를 찾아라" → 실제로 없음 → Gemini 가 공백을 가짜 citation 으로 메꿈. 이 경우 "literature 가 undisputed" 라는 valid finding 을 가짜 paper 로 denial 하게 됨.

**탐지 전략**: wrapper 레벨 탐지 **완전 불가** (stdout 깔끔, status completed, capacity 정상). **Skeptic URL verification 이 유일한 방어선**. Session 3 에서 Attack 1b (`research-skeptic.md` heuristic 1~6) 가 4/4 건 모두 잡아냄. 단 현 heuristic 은 "URL exists but content mismatch" (A2, B1) 를 직접 커버 안 함 → heuristic 7번 후보 추가 필요 (`research-skeptic.md` 개정).

**Deep Research B 루프에의 영향**: CRITICAL. Attack 1b 가 없었다면 Session 3 최종 보고서 13 claim 중 4 건 (31%) 이 fabricated URL 로 인용될 뻔했다. **Session 3 는 #12 의 존재를 실측한 첫 세션이며, B 패턴 (Proposer + Skeptic + Judge) 가 이 failure mode 에 대한 유일한 구조적 방어선이라는 것을 증명했다** — Step 4b 에서 B 패턴 채택 결정의 핵심 근거.

**후속 작업**:
1. `research-skeptic.md` Attack 1b 에 heuristic 7 추가: "URL exists but resolves to unrelated content" (A2/B1 패턴)
2. Gemini query template 강화: "못 찾으면 빈 칸, 절대 fabricate 금지" 를 Proposer 쿼리 공통부에 명시 (현재는 "Do NOT rely on blog posts" 수준)
3. 장기적으로 Gemini CLI upstream 에 "null citation" 옵션 요청 고려

**관련**: #10 (capacity-driven fabrication), Session 3 관찰 메타 노트 (`examples/deep-research.md` §"새 failure mode 발견").
