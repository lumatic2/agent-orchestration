...
bash "$ORCH_SCRIPT" gemini "$COLLECT_PROMPT" "$COLLECT_LOG_NAME" >/dev/null 2>&1 || true

COLLECT_LOG="$(ls -t "$LOGS_DIR"/gemini_"$COLLECT_LOG_NAME"_*.txt 2>/dev/null | head -1 || true)"
...
bash "$ORCH_SCRIPT" gemini "$CLASSIFY_PROMPT" "$CLASSIFY_LOG_NAME" >/dev/null 2>&1 || true

CLASSIFY_LOG="$(ls -t "$LOGS_DIR"/gemini_"$CLASSIFY_LOG_NAME"_*.txt 2>/dev/null | head -1 || true)"
