#!/usr/bin/env bash
# patch-gemini-companion.sh — Windows 환경에서 gemini-companion.mjs의 spawnSync 호환성 패치
#
# 문제: buildCmdStr + shellQuote가 Windows cmd.exe에서 백슬래시 경로를 깨뜨림
# 해결: spawnSync(bin, argsArray, {shell:true}) 배열 방식으로 전환, prompt만 shellQuote
#
