...
# Windows: shell=True + CREATE_NEW_PROCESS_GROUP (MSYS2 경로 변환 필요)
# Mac/Linux: start_new_session=True + os.killpg()
_run_with_timeout() {
  local timeout_sec="$1"; shift
  python3 - "$timeout_sec" "$@" << 'PYEOF'
import subprocess, sys, os, signal, shlex
timeout = int(sys.argv[1])
cmd = sys.argv[2:]
try:
    if os.name == 'nt':
        # Windows: shell=True로 cmd.exe 경유 → MSYS2 bash가 경로 인식 가능
        cmd_str = ' '.join(shlex.quote(c) for c in cmd)
        p = subprocess.Popen(cmd_str, stdout=subprocess.PIPE, stderr=subprocess.DEVNULL,
                             shell=True, creationflags=subprocess.CREATE_NEW_PROCESS_GROUP)
    else:
        # Mac/Linux: 프로세스 그룹 분리
