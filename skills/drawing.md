...
$ARGUMENTS가 비어 있으면 아래 메시지 출력 후 대화 시작:

```
🔗 다른 크리에이티브 작업에서 이어왔나요?
✏️ 글 → 제목·핵심 문단 붙여넣기
🎵 음악 → Suno 프롬프트·분위기 설명 붙여넣기
(없으면 Enter로 건너뛰기)
```

**연계 컨텍스트가 있으면:**
- 글에서 이어온 경우: 주제·감정·핵심 이미지어 추출 → 이미지 타입 자동 추천 + 프롬프트 소재 반영
- 음악에서 이어온 경우: 장르·분위기·악기 추출 → 시네마틱/앨범아트 추천 + 시각 분위기 반영
- 컨텍스트 없으면: 아래 메뉴로 진행
...
🟣 Midjourney — 최고 퀄리티. 아트워크, 사진풍, 시네마틱 모두 강력 (추천)
🔶 Adobe Firefly — 상업용 안전. 저작권 걱정 없음, Adobe 연동
🟢 Nanobanana — 무료. Gemini 기반, 빠른 실험용
