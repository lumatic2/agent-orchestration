...
- 질문: "어떤 리서치를 할까요?" (header: "리서치 유형")
- A) 기본 리서치 — 구조화 리서치 노트 출력, Gemini 위임, 3~5분
- B) vault 저장 — 기본 리서치 + vault 10-knowledge/{domain}/ 에 .md 저장
- C) 논문 초안 — 리서치 노트 + A4 3~5페이지 학술 논문 초안, 5~10분
- D) 풀 파이프라인 — arXiv 수집 → 검증 → PDF 생성, 30~60분 (개입 2~3회)

선택 후 "어떤 주제를 연구할까요?" 라고 묻고 주제를 입력받아라.
입력받은 모드와 주제를 기반으로 아래 연구 프로세스를 시작한다.
