...
**연계 컨텍스트 자동 감지**: 현재 세션에서 이전 크리에이티브 결과물(글·이미지)이 있으면 AskUserQuestion을 호출해라:
- 질문: "[제목/주제] 작업이 이어지고 있네요. 이 컨텍스트를 음악 방향에 반영할까요?"
- A) 반영 — 주제·감정·리듬감을 장르/분위기에 자동 연결
- B) 새로 시작 — 이전 작업과 독립적으로 진행

반영 로직:
- 글 반영: 주제·감정·리듬감 → 장르/분위기 자동 추천 + 가사 소재 반영
- 이미지 반영: 시각 분위기·색감·스타일 → 어울리는 사운드스케이프/장르 추천

컨텍스트 없거나 B 선택 시 → AskUserQuestion을 호출해라:
- 질문: "어떤 음악을 만들까요?" (header: "장르")
- A) 힙합 / R&B — 트랩·드릴·붐뱁·재즈힙합·로파이 / 트랩소울·네오소울·펑크
- B) 록 / 인디 — 인디·얼터너티브·팝록·포스트록
- C) 일렉트로닉 / 재즈 — EDM·하우스·테크노 / 재즈힙합·퓨전재즈·보사노바
- D) K-POP / 팝 / 기타 — 발라드·댄스팝·클래식·앰비언트·월드뮤직

카테고리 선택 후 구체적인 서브장르를 대화로 좁힌다.

`--quick`이면 "바로 만들기" 경로로 즉시. `--album`이거나 대화 중 "앨범"을 언급하면 Step 1-A로.

## 1. 설계 대화

사용자의 초기 설명을 받으면 AskUserQuestion을 호출해라:
- 질문: "{장르/분위기 요약}군요. 어떻게 진행할까요?" (header: "진행 방식")
- A) 바로 만들기 — 지금 정보로 즉시 프롬프트 생성 (빠르게, Recommended)
- B) 더 구체화하기 — BPM·악기·보컬·가사 등 추가 설정
