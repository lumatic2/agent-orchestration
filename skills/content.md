...
```bash
"ls -lt ~/projects/content-automation/outputs/ | head -10"
```
...
```bash
"cat ~/projects/content-automation/outputs/[파일명].json"
```

4. 사용자가 파이프라인을 수동 실행하고 싶다면:
```bash
# dry-run (Telegram/API 호출 없이 전체 흐름 검증)
"cd ~/Desktop/content-automation && python3 scheduler.py --dry-run --platform youtube"

# 실제 실행
"cd ~/Desktop/content-automation && python3 scheduler.py --platform youtube"
```
