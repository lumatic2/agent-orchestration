...
현황 보고 후 AskUserQuestion을 호출해라:
- 질문: "다음 작업을 선택하세요." (header: "액션")
- A) 스크리닝 실행 — 현재 전략 기준으로 종목 스크리닝 1회
- B) DART 캐시 갱신 — 재무 데이터·PER·배당 최신화
- C) 봇 재시작 — LaunchAgent 재시작
- D) 전략/실전 전환 — 전략 변경 또는 KIS 실계좌 전환
